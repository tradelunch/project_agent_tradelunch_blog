    ...
    async def initialize(self):
        ...
        try:
            ...
            self.pm = ProjectManagerAgent(
                llm_model=MODEL_NAME, base_url=OLLAMA_BASE_URL
            )
...
[cyan]File Processing:[/cyan]
  upload <file>         - Upload a blog post
  process <file>        - Process with metadata extraction
  analyze <file>        - Analyze content only (no upload)
...
"""
    ...
    async def process_command(self, user_input: str):
        ...
        # 파일 처리 명령어
            ...
            if not args:
                self.console.print(f"[red]Usage: {command} <file_path>[/red]")
                return
